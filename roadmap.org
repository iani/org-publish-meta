...
3. A mechanism for executing lisp code in those templates, by marking the code with tag =<lisp>=.

There are two types of templates in o-blog:

1. Separate files stored in a template directory, and included by
2. Snippets
