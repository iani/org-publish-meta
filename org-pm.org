...
#+BEGIN_SRC elisp
    (let ((def-node (org-map-entries '(cadr (org-element-at-point)) "PROJECT_DEFS"))
        (project)
        (plist)
        (buffer (get-buffer-create "*def*")))
    (set-buffer buffer)
  ...
  to the id of the main project."
    ...
    (let* ((p-name (car project))
      ...
      (setq static-project
            (-flatten (-map
                       (lambda (pair)
                         (list (replace-regexp-in-string "^static-" "" (car pair))
                               (cadr pair)))
                       (-filter
                        (lambda (pair) (string-match "^static-" (car pair)))
                        (-partition 2 p-def)))))
...
This command parses the current file in order to do 3 things:
...
** TODO org-pm-query-make-project-template MUST REVIEW! FIXME!

REWRITE THIS TO use org-pm-make-project-template.
...
Pass these as arguments to org-pm-make-project-template.
...
#+BEGIN_SRC emacs-lisp
  ...
  Skip those static and combined projects created by the system,
  i.e. projects whose ids end in -static and -combined."
    ...
    (let ((buffer (get-buffer-create "*org-pm-project-definitions*"))
      ...
      (dolist (project org-publish-project-alist)
        ...
        (unless (or
                 (string-match "^static-" node-id)
                 (string-match "^combined-" node-id))
         (insert "\n** "
                 (car project)
...
** org-pm-default
:PROPERTIES:
:ID:       7F35BA92-3A62-4E62-AB79-123F3B61174E
:END:
