...
**** Initial Test version: Remove cache-requiring filters
...
#+BEGIN_SRC elisp
  (defun org-publish-org-to (buffer path plist)
      ...
      (prog1 (with-current-buffer buffer
               (let ((output-file path)
                 ...
                 (org-export-to-file 'html output-file
                   ...
                   ;; in cache anyway.
                   plist)))
        ;; DO NOT Remove opened buffer in the process.
        )))
#+END_SRC

**** With Yaml: For final version
