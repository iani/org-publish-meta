...
#+BEGIN_SRC emacs-lisp
  ...
  Immediately renders sections that belong to html-projects.
...
#+BEGIN_SRC elisp
  ...
  PLIST is the property list for the given project."
    ...
    (let* ((org-inhibit-startup t)
      ...
      ;; DISABLING THIS: DEBUGGING!
      (unless (file-exists-p pub-dir) (make-directory pub-dir t))
      (if t (with-current-buffer work-buffer
      ...
      (message "work buffer:  %s" work-buffer)
      ))
...
#+BEGIN_SRC elisp
  (defun org-pm-add-yaml-front-matter (string backend info)
    ...
    (when (org-export-derived-backend-p backend 'html)
      (let ((yaml (plist-get info :yaml-front-matter))
            (if yaml (concat yaml string) string)))))
