...
** Code: org-pm-make-yaml-front-matter


#+BEGIN_SRC emacs-lisp
  (defun org-pm-make-yaml-front-matter (project-plist &optional section-plist)
  ...
  If section-plist is nil, then the properties are obtained from the buffer's
  global properties.  Otherwise, they are obtained from section-plist."

    (when (plist-get project-plist :body-only)
      (insert "---\n")
      (multiple-value-bind
          (author categories comments date external-url
                  layout permalink published tags title)
          (if section-plist
              (values-list
               (list
                () ;; get author
          ...
          ((property-getter
               (if section-plist
                   (lambda (prop) (plist-get section-plist prop))
                 (lambda (prop) (org-get-header-property prop t))))
              (date (apply property-getter '(":DATE")))
              (layout (apply property-getter '(":LAYOUT")))
              (title (if section-plist
                         (plist-get section-plist :raw-value)
                       (let ((ttl (org-get-header-property "TITLE")))
                         (if ttl ttl
                           (file-name-sans-extension
                            (file-name-nondirectory (buffer-file-name))))))))
        (insert
         (format "layout: %s\n" (if layout layout (if date "blog" "default"))))
