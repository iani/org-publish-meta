...
- Do org-scan-tags using a matcher constructed from =tagsmatch=, and a function that does the following:
  ...
  - add the exported section to =org-pm-section-exports=,
    ...
    entry as key, and use =assoc-add= to add path-project pairs to it,
...
#+BEGIN_SRC elisp
  (defun org-pm-make-tagmatch (heading)
  "parse a heading of the form
  <project> <tagmatch> <folder> <layout>
  and produce a form (what exactly???) for matching the headings that
  match tagmatch + constructing filename paths for them to be exported."
