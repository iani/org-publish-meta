...
- DONE: See org-pm-get-tag-matching-lists: Split the header string at " " and multiple-bind the variables =project= =tagsmatch= =folder= =layout= to the resulting values.
- ONLY THE NON-NIL LISTS ARE RETURNED BY org-pm-get-tag-matching-lists.  SO OK: When =project-plist= is not nil, proceed with following:
- For each of the tag-match project specification lists do the following:
...
- Do org-scan-tags using a matcher constructed from =tagsmatch=, and a function that does the following:
  ...
  - create export target path by =concat= of following:
    ...
    - construct filename with =org-pm-make-filename=, using:
  ...
  - add the exported section to =sections-with-paths=, which was
    started by the first part of =org-pm-export=
    This has now been tested with custom function =assoc-add2=
    ("Add element to the sublist of alist which starts with key, but insert element2 between key and the rest of the list.")
  - Since the export of both underscore matched and tag-match matched section is now completed, adding to =sections-with-paths= does not interfere at this stage.
  - Add =sections-with-paths= to the global variable =org-pm-section-exports=.
  - Save org-pm data (which also saves the updated =org-pm-section-exports=).
    ...
    (save-excursion
      (save-restriction
        ...
        (let* ((sections-with-paths (org-pm-get-section-project-paths))
          ...
          ;; below test worked. we can proceed to call org-pm-export-tag-matched-sections
          ;; using sections-with-paths available through lexical binding
          ;; (org-pm-test-lexical-binding-of-sections-with-paths)
