...
**** COMMENT org-pm-export Top level: publish sections to html or source
...
#+BEGIN_SRC emacs-lisp
 ...
 #+END_SRC
  ...
  Now let's try the new assoc-add

 #+BEGIN_SRC elisp
   (assoc-add2 '((1 "id" ("path". "project"))) 1 '("path2" . "project2") "yes---id2")
 #+END_SRC
...
****** org-pm-sectin-project-paths version 2: using org-scan-tags
...
******** Tags mapped to projects in org mode sections

The heading of a subsection of a sectiong tagged ORG_PM_PROJECT_TAGS specifies project, tag matches that go to the project, folder, and layout:

<project> <tagmatch> <folder> <layout>

Given the above, proceed as follows:

- Split the header string at " " and multiple-bind the variables =project-name= =tagsmatch= =folder= =layout= to the resulting values.
- Provide defaults "" for folder and "default" for layout, if these are nil.
- Get the project plist from =project-name=, put it in =project-plist=.
- When =project-plist= is not nil, proceed with following:
- Do org-scan-tags using a matcher constructed from =tagsmatch=, and a function that does the following:
  - put (cadr (org-element-at-point)) into =section-plist=
  - create export target path by =concat= of following:
    - property =publishing-directory= of =project-plist=
    - property =folder= of =section-plist= or variable =folder=
    - filename constructed with =org-pm-make-filename=, using:
      - property =:raw-value= of =section-plist=
      - property =:DATE:= of =section-plist=
  - make =target-buffer= with =org-pm-make-section-buffer=,
    using current buffer as =origin-buffer=,
    and current =(point)= as =section-begin=.
  - do the rest of =org-pm-export-1-section-to-projects=,
    starting from line: =(if (equal (plist-get plist :publishing...)))=
  - add the exported section to =org-pm-section-exports=,
    This is going to be complex:
    The order of creating the component-list of sections/projects/paths
    is different than that for the underscore tag matching.
    So we could create the components list as an assoc list
    with the point of the start of each
    entry as key, and use =assoc-add2= to add path-project pairs to it,
    and then when all the projects have been processed,
    we merge this components list with the one created by the
    underscore-match export.
    So do the tagsmatch type of export first,
    then the underscore-match type of export.

    Note: Look at own function =assoc-add=
    (""Add element to the sublist of alist which starts with key.")
    with path of file of current buffer as key.

IMPORTANT: Check that assoc-add does not cause conflicts with the
other use of the section list by the underscore-tag based export mechanism.

#+BEGIN_SRC elisp
  (defun org-pm-make-tagmatch-list ()
  ...
  If a project named in a heading has no definition, then skip that heading."
    (save-excursion
      (save-restriction
        ...
        (let (entries)
          (org-map-entries
           '(lambda () (cadr (org-element-at-point)))
           "ORG_PM_EXPORT_TAGS")
...
******** COMMENT Test                                     :ORG_PM_EXPORT_TAGS:
...
#+RESULTS:
| nil                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |   2 |       |       |      |     |     |      |      |      |                                                                                                                                                                                                                                                                               |   |
| (octopress_pm_test :project-name octopress_pm_test :node-id 5B6A6F23-AB07-4F56-BFC8-783634E3D8A9 :node-filename /Users/iani2/Dropbox/000WORKFILES/org/monitoring/log.org :project-id /Users/iani2/Dropbox/000WORKFILES/org/monitoring/log.org::#5B6A6F23-AB07-4F56-BFC8-783634E3D8A9 :last-updated [2014-01-23 Thu 19:26] :body-only t :publish-to-jekyll t :base-directory ~/Documents/Dev/Web/org-pm-test01 :base-extension org :recursive t :publishing-directory ~/Documents/Dev/Web/org-pm-test01 :publishing-function org-html-publish-to-html :headline-levels 4 :section-numbers nil :with-toc nil :with-author t :with-creator t :with-emphasize t :with-sub-superscript nil :with-tables t :with-tags t :with-tasks t :with-todo-keywords nil :with-latex t :with-drawers nil :html-head-include-default-style nil :html-head-include-scripts nil :toc-heading Contents :static-base-directory ~/Documents/Dev/Web/octopress-pm-test-org/static :static-publishing-directory ~/Documents/Dev/Web/octopress-pm-dev-html/static :static-base-extension css\ | js\ | less\ | scss\ | php\ | rb\ | py\ | pdf\ | jpg\ | gif\ | png :static-publishing-function org-publish-attachment :static-recursive t :yaml-header ---\ntitle: address mtg andrea cohen: 01/05/14 13:10:17\nlayout: default\nauthor: Iannis Zannos\ndate: 2014-01-23 19:26:00 +0200\ntags:\n- friends\n- travel\n- paris\n- test\n---\n) | 1 |

********* octopress_pm_test 1

********* Subtree 2

******** Tags specifying projects enclosed in underscores

A tag enclosed in underscores (=_=) specifies the project, folder, filename.
This is the first method implemented in org-pm
...
|-------------------------------+---------------------------------------------|
| H-m s                         | org-pm-open-source-of-this-file             |
| H-m t                         | org-pm-open-target-of-this-file             |
| H-m f                         | org-pm-list-exported-files (current buffer) |
| C-u H-m f                     | org-pm-list-exported-files (all)            |
|-------------------------------+---------------------------------------------|
