...
See function org-pm-publish-buffer-to in next session.
...
*** TODO org-pm-publish-buffer-to

Test case for =org-pm-publish-buffer-to=.  Shows that it works if we initialize cache.
...
(let* ((project (assoc "octopress_pm_test"  org-publish-project-alist))
  ...
  (org-pm-publish-buffer-to (get-buffer "test") path plist)
  )
#+END_SRC

**** QUOTE Version to use: Remove cache-requiring filters
...
This works with the version below:
...
#+BEGIN_SRC elisp
  ...
  PROJECT is the name+property list for the given project.
  ...
  directory."
    ...
    (let* ((pname (car project))
      ...
      (when (plist-get plist :export-to-jekyll)
        ;; if this is for jekyll, then make yaml header.
        ;; do this in a separate buffer, leaving the original untouched.
          (setq get-rid-of-buffer-when-done t)
          (setq work-buffer (get-buffer "*org-pm-publish-buffer*"))
      ...
      (prog1 (with-current-buffer work-buffer
        ...
        ;; Remove working-buffer if it is private to this function
        (if get-rid-of-buffer-when-done (kill-buffer work-buffer)))))

#+END_SRC

#+RESULTS:
: org-pm-publish-buffer-to
