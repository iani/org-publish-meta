...
* Add/remove projects to files/sections
...
* Export files/sections to projects
...
* Create Octopress Headers

** Notes/Outline

A Jekyll/Octopress header is by default added to the exported file if the project has property =body-only= set to =t=.
(Reference: see http://orgmode.org/worg/org-tutorials/org-jekyll.html#sec-4)

- layout is default if no date present
- layout is blog if date present
- layout can be (overriden/) specified by property LAYOUT
- title is:
  - TITLE property in case of file
  - header of section in case of section
- comments is true unless overridden by property COMMENTS
- categories is taken from tags, minus project tags
- Other entries in header?  Which properties to look for?
...
** Code: org-pm-make-octopress-header

(Or should it be: org-pm-make-jekyll-header?)
